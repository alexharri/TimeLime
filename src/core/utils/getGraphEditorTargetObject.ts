import {
  CONTROL_POINT_DISTANCE_TRESHOLD,
  KEYFRAME_DISTANCE_TRESHOLD,
} from "~/core/constants";
...
export const getGraphEditorTargetObject = (
  ...
  for (let i = 0; i < keyframes.length - 1; i += 1) {
    ...
    const cp0_dist = cp0
      ? getDistance(normalToViewport(cp0), viewportMousePosition)
      : Infinity;
    const cp1_dist = cp1
      ? getDistance(normalToViewport(cp1), viewportMousePosition)
      : Infinity;
