import { ActionOptions, MousePosition, Rect, SomeMouseEvent } from "timelime/types";
...
interface MouseDownMoveActionOptions<K extends Key> {
  ...
  mouseMove: (params: RequestActionParams, options: MouseMoveOptions<Record<K, boolean>>) => void;
...
export const mouseDownMoveAction = <K extends Key>(
  ...
  if (options.globalToNormal) {
    ...
    translate = (vec) => {
      const x = options.globalToNormalX ? options.globalToNormalX(vec.x) : vec.x;
      const y = options.globalToNormalY ? options.globalToNormalY(vec.y) : vec.y;
      return Vec2.new(x, y);
    };
  } else {
    translate = (vec) => vec;
  ...
  const fn: RequestActionCallback = (params) => {
    ...
    const tick = () => {
      ...
      const keyDownMap = (lastKeyDownMap = keys.reduce<Record<K, boolean>>((acc, key) => {
      ...
      const getOptions = () => {
        ...
        const mousePosition: MousePosition = {
          ...
          viewport: options.viewport
            ? globalMousePosition.sub(Vec2.new(options.viewport.left, options.viewport.top))
            : globalMousePosition,
  ...
  requestAction({ userActionOptions, shouldAddToStack: options.shouldAddToStack }, fn);
};
